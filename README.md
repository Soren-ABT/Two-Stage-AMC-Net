...
| 特性 | 说明 |
|------|------|
| **多模态输入** | 将原始 I/Q 信号扩展为 4 通道（I, Q, Amplitude, Phase），降低特征学习难度 |
| **两阶段课程训练** | Stage 1 在高 SNR（10~30 dB）数据上预训练，Stage 2 在全 SNR（-20~30 dB）数据上微调 |
| **残差网络** | 基于 1D ResNet-18 骨架，支持更深层特征提取，缓解梯度消失 |
| **数据增强** | 在线进行随机幅度缩放、相位旋转、高斯加噪，提升泛化能力 |
| **显式正则化** | 标签平滑（Label Smoothing = 0.1） + AdamW 权重衰减 + 卷积块 Dropout |
| **梯度裁剪** | 防止训练不稳定，限制梯度范数 ≤ 1.0 |
| **余弦退火** | CosineAnnealingLR 学习率调度，平滑收敛 |
| **早停机制** | 验证损失连续不降则自动终止训练，避免过拟合 |
| **Per-SNR 评估** | 按 26 个 SNR 点分别统计准确率，绘制 Accuracy vs SNR 曲线 |
| **PDF 报告** | 训练结束后自动生成实验报告 |
